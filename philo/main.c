...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/22 17:20:33 by salaverd          #+#    #+#             */
/*   Updated: 2022/04/23 16:34:16 by salaverd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_philosophers(t_main *instance)
{
	...
	while (i < instance->amount)
	{
		...
		gettimeofday(&instance->philosophers[i].create_time, NULL);
...
int	main(int argc, const char **argv)
{
	...
	if (init_args(&utils, argc, argv)
		|| init_philosophers(&utils)
		|| init_mutexes(&utils))
		return (1);
	if (start(&utils))
		return (1);
	pthread_mutex_lock(&utils.die_mutex);
	pthread_mutex_unlock(&utils.die_mutex);
	clear(&utils);
	exit(0);
}
